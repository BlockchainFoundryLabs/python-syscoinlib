...
# Copyright (C) 2011-2015 The python-syscoinlib developers
#
...
import syscoin
from syscoin.core import COIN, x, lx, b2lx, CBlock, CBlockHeader, CTransaction, COutPoint, CTxOut
...
class BaseProxy(object):
    ...
    def __init__(self,
        ...
        if service_url is None:
            # Figure out the path to the syscoin.conf file
            if sys_conf_file is None:
                if platform.system() == 'Darwin':
                    sys_conf_file = os.path.expanduser('~/Library/Application Support/Syscoin/')
                elif platform.system() == 'Windows':
                    sys_conf_file = os.path.join(os.environ['APPDATA'], 'Syscoin')
                else:
                    sys_conf_file = os.path.expanduser('~/.syscoin')
                sys_conf_file = os.path.join(sys_conf_file, 'syscoin.conf')

            # Extract contents of syscoin.conf to build service_url
            with open(sys_conf_file, 'r') as fd:
                # Syscoin Core accepts empty rpcuser, not specified in sys_conf_file
                conf = {'rpcuser': ""}
                ...
                if service_port is None:
                    service_port = syscoin.params.RPC_PORT
                conf['rpcport'] = int(conf.get('rpcport', service_port))
                conf['rpchost'] = conf.get('rpcconnect', 'localhost')

                if 'rpcpassword' not in conf:
                    raise ValueError('The value of rpcpassword not specified in the configuration file: %s' % sys_conf_file)
...
class RawProxy(BaseProxy):
    """Low-level proxy to a syscoin JSON-RPC service
    ...
    def __getattr__(self, name):
        ...
        # Make debuggers show <function syscoin.rpc.name> rather than <function
        # syscoin.rpc.<lambda>>
        f.__name__ = name
        return f
...
class Proxy(BaseProxy):
    """Proxy to a syscoin RPC service

    Unlike ``RawProxy``, data is passed as ``syscoin.core`` objects or packed
    ...
    way. Assumes Syscoin Core version >= v0.13.0; older versions mostly work,
    ...
    def __init__(self,
        ...
        out of the file ``sys_conf_file``. If ``sys_conf_file`` is not
        specified, ``~/.syscoin/syscoin.conf`` or equivalent is used by
        default.  The default port is set according to the chain parameters in
        use: mainnet, testnet, or regtest.

        Usually no arguments to ``Proxy()`` are needed; the local syscoind will
        be used.
    ...
    def getaccountaddress(self, account=None):
        """Return the current Syscoin address for receiving payments to this
        account."""
        r = self._call('getaccountaddress', account)
        return CSyscoinAddress(r)
    ...
    def getnewaddress(self, account=None):
        """Return a new Syscoin address for receiving payments.
    ...
    def getreceivedbyaddress(self, addr, minconf=1):
        ...
        addr    - The address. (CSyscoinAddress instance)
    ...
    def submitblock(self, block, params=None):
        ...
        params is optional and is currently ignored by syscoind. See
        https://en.syscoin.it/wiki/BIP_0022 for full specification.
        """
    ...
    def validateaddress(self, address):
        ...
        if r['isvalid']:
            r['address'] = CSyscoinAddress(r['address'])
        if 'pubkey' in r:
