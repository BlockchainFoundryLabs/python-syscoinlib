...
unlikely to match Satoshi Syscoin exactly. Think carefully before using this
module.
"""
...
import syscoin.core
import syscoin.core._bignum
import syscoin.core.key
import syscoin.core.serialize
...
from syscoin.core.script import *
...
class EvalScriptError(syscoin.core.ValidationError):
...
def _CastToBigNum(s, err_raiser):
    v = syscoin.core._bignum.vch2bn(s)
...
def _CheckMultiSig(opcode, script, stack, txTo, inIdx, flags, err_raiser, nOpCount):
    ...
    # Note how Syscoin Core duplicates the len(stack) check, rather than
...
def _UnaryOp(opcode, stack, err_raiser):
    ...
    stack.append(syscoin.core._bignum.bn2vch(bn))
    ...
    for (sop, sop_data, sop_pc) in scriptIn.raw_iter():
        ...
        if sop <= OP_PUSHDATA4:
            ...
            if sop == OP_1NEGATE or ((sop >= OP_1) and (sop <= OP_16)):
                v = sop - (OP_1 - 1)
                stack.append(syscoin.core._bignum.bn2vch(v))
                ...
                check_args(1)
                stack.append(syscoin.core.serialize.Hash160(stack.pop()))

            elif sop == OP_HASH256:
                check_args(1)
                stack.append(syscoin.core.serialize.Hash(stack.pop()))
...
class VerifyScriptError(syscoin.core.ValidationError):
    pass
...
class VerifySignatureError(syscoin.core.ValidationError):
    pass
