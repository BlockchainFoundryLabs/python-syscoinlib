python-syscoinlib release notes
===============================
...
* The alias `JSONRPCException = JSONRPCError` has been removed. This alias was
  added for compatibility with v0.4.0 of python-syscoinlib.
...
* Where appropriate, `RPC_INVALID_ADDRESS_OR_KEY` errors are now caught
  ...
  initial startup previously python-syscoinlib would incorrectly raise
  ...
  all!) of the types of RPC errors Syscoin Core returns.

Bugfixes:

* Fixed a spurious `AttributeError` when `syscoin.rpc.Proxy()` fails.
...
* RPC over SSL support removed to match Syscoin Core's removal of RPC SSL
  support in v0.12.0 If you need this, use an alternative such as a stunnel or
  a SSH tunnel.

* Removed SCRIPT_VERIFY constants ``syscoin.core.script``, leaving just the
  constants in ``syscoin.core.scripteval``; being singletons the redundant
  constants were broken anyway.

* SCRIPT_VERIFY_EVEN_S renamed to SCRIPT_VERIFY_LOW_S to match Syscoin Core's naming

* SCRIPT_VERIFY_NOCACHE removed as Syscoin Core no longer has it (and we never
...
* Signing and verification of Syscoin Core compatible messages. (w/ pubkey recovery)
* Tox tests
* Sphinx docs
...
Major fix: OpenSSL 1.0.1k rejects non-canonical DER signatures, which Syscoin
...
* No longer assumes syscoin.conf specifes rpcuser
...
attribute of the syscoin.core.key module, and the entire syscoin.core.bignum
...
* Improve syscoin address handling. P2SH and P2PKH addresses now get their own
  classes - P2SHSyscoinAddress and P2PKHSyscoinAddress respectively - and P2PKH
...
'python-syscoinlib-(version)', replacing the less specific '(version)' form
previously used.
